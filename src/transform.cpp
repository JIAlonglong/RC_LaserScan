...
public:
	//声明TF的聆听者、ROS句柄、scan的订阅者、变换后scan的发布者
	tf::TransformListener *tf_;
	ros::NodeHandle nh_;
	ros::Subscriber scan_sub_;
	ros::Publisher scan_pub_;
	//针对各自的情况需要更改的名字，自行更改
	const std::string scan_frame_name_ = "laser";
	const std::string odom_name_ = "my_odom";
	const std::string scan_sub_name_ = "scan";
	const std::string scan_pub_name_ = "new_scan";
...
bool LidarMotionCalibrator::getLaserPose(tf::Stamped<tf::Pose> &odom_pose, ros::Time dt, tf::TransformListener *tf_)
{
	...
	try
	{
		...
		if (!tf_->waitForTransform(odom_name_, scan_frame_name_, dt, ros::Duration(0.1))) // 0.15s 的时间可以修改
		{
...
void LidarMotionCalibrator::scan_cal_pub(const std::vector<double> &ranges, ros::Time pubTime, double scan_time)
{
	...
	//改
	tempScan.angle_min = -3*PI/4;
	tempScan.angle_max = 3*PI/4;
	...
	tempScan.range_min = 0.01;
	tempScan.range_max = 20;
	...
	for (size_t i = 0; i < ranges.size(); ++i)
	{
		...
		tempScan.intensities[i] = 15.0; //设置强度
	}
	scan_pub_.publish(tempScan); //发布新scan
}
